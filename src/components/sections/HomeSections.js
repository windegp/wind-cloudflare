  ...
  return (
    <section ref={sectionRef} className="bg-white overflow-hidden pt-8 pb-16" dir="rtl">
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className={`flex justify-center gap-6 md:gap-8 mb-8 px-4 transition-all duration-[600ms] delay-100 ease-[cubic-bezier(0.22,1,0.36,1)] ${isSectionVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3'}`}>
          {tabs.map((tab, idx) => (
            <button
              key={idx}
              onClick={() => setActiveTab(idx)}
              className={`pb-2 text-[15px] md:text-[16px] relative transition-colors duration-300 font-tajawal whitespace-nowrap ${
